...
@pytest.mark.parametrize(
    ...
    (
        ...
        pytest.param(
            "SELECT col1 FROM table1 |> SELECT AS STRUCT col1", id="pipe_select_select_as_struct"
        ),
        pytest.param(
...
@pytest.mark.parametrize(
    "sql", (pytest.param("FROM table1 | > SELECT col1", id="pipe_with_space"),)
)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        pytest.param(
            "FROM table1 |> SET col1 = POW(col1, 2),", id="pipe_set_single_trailing_comma"
        ),
        pytest.param(
            "FROM table1 |> SET col1 = POW(col1, 2), col2 = EXP(col1, 2)", id="pipe_set_multi"
        ),
    ),
)
...
@pytest.mark.parametrize(
    "sql", (pytest.param("FROM table1 |> RENAME col1.nested AS col2", id="pipe_rename_nested"),)
)
def test_pipe_rename_error(sql, assert_parse_tree_error):
    assert_parse_tree_error(sql)


@pytest.mark.parametrize("sql", (pytest.param("FROM table1 |> AS table2", id="pipe_as_single"),))
def test_as_ok(sql, assert_parse_tree):
    assert_parse_tree(sql)


@pytest.mark.parametrize(
    "sql", (pytest.param("FROM table1 |> WHERE col1", id="pipe_where_single"),)
)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        pytest.param(
            ...
            id="pipe_agg_group_by_order_by",
        ),
        pytest.param(
            "FROM table1 |> AGGREGATE SUM(col1) AS col2 GROUP AND ORDER BY col3",
            id="pipe_agg_group_and_order_by",
        ),
        pytest.param(
            "FROM table1 |> AGGREGATE SUM(col1) AS col2 GROUP BY ()", id="pipe_agg_group_by_empty"
        ),
        pytest.param(
...
@pytest.mark.parametrize(
    ...
    (
        ...
        pytest.param(
            "FROM table1 |> ORDER BY col1 NULLS FIRST, col2 NULLS LAST", id="order_by_multi_nulls"
        ),
        pytest.param(
            "FROM table1 |> ORDER BY col1 ASC NULLS FIRST, col2 DESC NULLS LAST",
            id="order_by_multi_kitchen_sink",
        ),
    ),
)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        pytest.param(
            ...
            id="set_operation_distinct_multi",
        ),
        pytest.param(
            "FROM table1 |> EXCEPT ALL BY NAME (SELECT 1 as col1)", id="set_operation_by_name"
        ),
        pytest.param(
            "FROM table1 |> UNION ALL CORRESPONDING (SELECT 1 as col1)",
            id="set_operation_corresponding",
        ),
        pytest.param(
            "FROM table1 |> UNION ALL STRICT CORRESPONDING (SELECT 1 as col1)",
            id="set_operation_strict_corresponding",
        ),
        pytest.param(
            "FROM table1 |> EXCEPT ALL BY NAME ON (col1) (SELECT 1 as col1)",
            id="set_operation_by_name_on_single",
        ),
        pytest.param(
        ...
        pytest.param(
            ...
            id="set_operation_strict_corresponding_by",
        ),
        pytest.param(
...
@pytest.mark.parametrize(
    ...
    (
        ...
        pytest.param(
            "FROM table1 |> JOIN table2 ON col1 = col2 AND col3 != col4", id="join_on_compound_expr"
        ),
...
@pytest.mark.parametrize(
    ...
    (
        ...
        pytest.param(
            "FROM table1 |> CALL APPENDS('1970-01-01') AS table2", id="call_alias_explicit"
        ),
...
@pytest.mark.parametrize(
    "sql", (pytest.param("FROM table1 |> TABLESAMPLE SYSTEM (10 percent)", id="tablesample"),)
)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        pytest.param(
            "FROM table1 |> UNPIVOT (col1 FOR quarter IN (q1, q2)) AS col2", id="unpivot_aliased"
        ),
    ),
)
